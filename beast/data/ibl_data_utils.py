...
def load_target_behavior(one, eid, target):
    ...
    try:
        if target == "wheel-speed":
            ...
            lick_times = licks["times"]
            beh_dict = {
                "times": lick_times, "values": None, "skip": False,
            }
            ...
            nose_speed = get_speed(lp, lp_times, view, feature="nose_tip")
            values = nose_speed
        ...
        if not target_dict["skip"]:
            ...
            if beh == "licks":
                ...
                if target_vals is None:
                    warnings.warn(
                        f"Behavior {beh} not found; skipping.",
                        UserWarning,
                        stacklevel=2,
                    )
                    continue
            ...
            behave_dict.update(
                {
                    beh: np.array(target_vals_list, dtype=object).squeeze()
                }
            )
...
def align_data(
    ...
    for beh in binned_behaviors.keys():
        beh_mask = []
        for x in binned_behaviors[beh]:
    ...
    for beh in beh_names:
        ...
        aligned_binned_behaviors.update(
            {beh: np.delete(beh_vals, bad_trial_idxs, axis=0)}
        )
        aligned_binned_behaviors[beh] = np.array([y for y in aligned_binned_behaviors[beh]], 
            dtype=float).reshape((num_trials, n_tbin, -1)
        )
